...
module Pulse
	class Synthesizer
		EXTRACT_SYSTEM = <<~PROMPT
			你是 Pulse，一个进度追踪助手。你的任务是从下面收集的员工更新中
			识别出每一个不同的议题、项目或主题。
		...
		SYNTHESIZE_TEMPLATE = <<~PROMPT
			你是 Pulse，一个为高管编写进度报告的助手。
			你将收到关于单个议题或项目的摘录，请用中文撰写简洁的状态报告段落。

			今天的日期是：%<today>s

			格式规则：
			- 严格按照下面的示例格式输出，注意空行位置
			- 第一行必须是 ### 加状态指示符加议题名称
			- 状态指示符：
			  🟢 正常 — 进展顺利
			  🟡 关注 — 需要关注 / 有风险
			  🔴 阻塞 — 受阻 / 偏离计划 / 逾期
			  ✅ 完成 — 已完成 / 已解决
			- 必须包含"负责人"行——从摘录中提取相关人员姓名，用 @ 标注
			- 必须包含"截止日期"或"计划日期"行——从摘录中提取，如果没有明确日期则写"待定"
			- 正文需要提供足够的上下文：当前在做什么、下一步计划、存在的风险或阻碍
			- 对于 ✅ 已完成的项目，在正文末尾增加一行 **回顾：** 总结完成情况
			- 简洁直接，只陈述事实，不要废话
			- 输出原始 markdown，不要代码块
		...
		SYNTHESIZE_WITH_PREVIOUS_TEMPLATE = <<~PROMPT
			...
			- 状态指示符：
			  ...
			  🔴 阻塞 — 受阻 / 偏离计划 / 逾期
			  ✅ 完成 — 已完成 / 已解决
			- 必须包含"负责人"行——从摘录中提取相关人员姓名，用 @ 标注；沿用上期信息
			- 必须包含"截止日期"或"计划日期"行——从摘录中提取；沿用上期信息；如果没有明确日期则写"待定"
			- 正文需要提供足够的上下文：当前在做什么、下一步计划、存在的风险或阻碍
			- 对于 ✅ 已完成的项目，在正文末尾增加一行 **回顾：** 总结完成情况
			- 简洁直接，只陈述事实，不要废话
			- 输出原始 markdown，不要代码块
