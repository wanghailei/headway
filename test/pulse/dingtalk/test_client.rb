# Tests for Pulse::DingTalk::Client. Verifies token fetching, caching,
...
class TestDingTalkClient < Minitest::Test
	...
	def test_raises_auth_error_on_failed_auth
		...
		client = Pulse::DingTalk::Client.new( app_key: "bad", app_secret: "bad" )
		assert_raises( Pulse::DingTalk::Client::AuthError ) do
	...
	def test_raises_auth_error_when_access_token_missing_from_response
		...
		client = Pulse::DingTalk::Client.new( app_key: "key", app_secret: "secret" )
		error = assert_raises( Pulse::DingTalk::Client::AuthError ) do
			client.get( "/v1.0/anything", connection: conn )
		end
	...
	def test_raises_api_error_on_failed_request
		...
		client = Pulse::DingTalk::Client.new( app_key: "key", app_secret: "secret" )
		assert_raises( Pulse::DingTalk::Client::APIError ) do
	...
	def test_raises_api_error_on_non_json_error_response
		...
		client = Pulse::DingTalk::Client.new( app_key: "key", app_secret: "secret" )
		error = assert_raises( Pulse::DingTalk::Client::APIError ) do
			client.get( "/v1.0/bad", connection: conn )
		end
	...
	def test_raises_on_missing_app_key
		assert_raises( ArgumentError ) do
			Pulse::DingTalk::Client.new( app_key: nil, app_secret: "secret" )
		end
	end

	def test_raises_on_missing_app_secret
		assert_raises( ArgumentError ) do
			Pulse::DingTalk::Client.new( app_key: "key", app_secret: nil )
		end
	end
end
